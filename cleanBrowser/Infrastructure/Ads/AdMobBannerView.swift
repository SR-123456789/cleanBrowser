...
struct AdMobBannerView: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> BannerView {
        ...
        configureRootViewController(for: bannerView)
        loadAdIfNeeded(into: bannerView, coordinator: context.coordinator, force: true)
        return bannerView
    }
    
    func updateUIView(_ uiView: BannerView, context: Context) {
        configureRootViewController(for: uiView)
        loadAdIfNeeded(into: uiView, coordinator: context.coordinator, force: false)
    }

    private func configureRootViewController(for bannerView: BannerView) {
        guard bannerView.rootViewController == nil,
    ...
    private func loadAdIfNeeded(into bannerView: BannerView, coordinator: Coordinator, force: Bool) {
        let canShowPersonalizedAds = attManager.canShowPersonalizedAds
        guard force
                || coordinator.lastCanShowPersonalizedAds != canShowPersonalizedAds
                || coordinator.lastAdUnitID != adUnitID
        else {
            return
        }
    ...
    final class Coordinator {
        var lastCanShowPersonalizedAds: Bool?
